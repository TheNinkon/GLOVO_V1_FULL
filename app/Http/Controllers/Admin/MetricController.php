...
class MetricController extends Controller
{
    ...
    public function list(Request $request)
    {
        $query = Metric::query()->from('glovo_metrics as m')
            ...
            ->leftJoin('assignments as ass', function ($join) {
                $join->on('ass.account_id', '=', 'a.id')
                    // CORRECCIÓN: La lógica del JOIN ahora es más robusta
        ...
        if ($request->filled('date_from') && $request->filled('date_to')) {
            $query->whereBetween('m.fecha', [$request->date_from, $request->date_to]);
        }
        ...
        if ($request->filled('rider_name')) { // NUEVO FILTRO
    ...
    public function kpis(Request $request)
    {
        ...
        // CORRECCIÓN: Si el filtro de rider_name está activo, necesitamos los joins
        ...
        if ($request->filled('date_from') && $request->filled('date_to')) {
            $query->whereBetween('fecha', [$request->date_from, $request->date_to]);
        }
        if ($request->filled('city')) {
